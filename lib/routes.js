...
module.exports = ({ cache, config }) => {
  ...
  // Proxy pobrania konkretnego pliku wydania po nazwie (np. .nupkg dla Squirrel.Windows),
  // z autoryzacją tokenem — dzięki temu działa dla repozytorium prywatnego.
  exports.downloadAsset = async (req, res) => {
    const { filename } = req.params
    const latest = await loadCache()

    if (!shouldProxyPrivateDownload) {
      send(res, 404, 'Private download not configured')
      return
    }

    if (!latest.assets || !latest.assets[filename]) {
      send(res, 404, 'No such file in the latest release')
      return
    }

    proxyPrivateDownload(latest.assets[filename], req, res)
  }
