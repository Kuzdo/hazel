...
module.exports = class Cache {
  ...
  async cacheReleaseList(url) {
    ...
    // Pobranie pliku RELEASES adresem API z tokenem (działa też dla repo prywatnego).
    ...
    // Zostawiamy nazwy plików .nupkg bez zmian — Squirrel pobierze je względem
    // adresu RELEASES, czyli przez nasz endpoint proxujący (/update/win32/:version/:plik).
    return content
  }
  ...
  async refreshCache() {
    ...
    this.latest.files = {}
    // Wszystkie pliki wydania wg nazwy — do proxowania (np. .nupkg dla Windows)
    this.latest.assets = {}

    for (const asset of release.assets) {
      const { name, browser_download_url, url, content_type, size } = asset

      // zapamiętaj każdy plik wydania po nazwie (api_url do autoryzowanego pobrania)
      this.latest.assets[name] = { name, api_url: url }
